...
class Server {
  ...
  public Server serve(Customer customer) {
    Server newServer = new Server(this.id, Optional.of(customer), this.waitingCustomer);
    newServer = newServer.getWaitingCustomer().filter(c -> c.equals(customer))
        .map(c -> new Server(this.id, Optional.of(c), Optional.empty())).orElse(newServer);
    return newServer;
  }
